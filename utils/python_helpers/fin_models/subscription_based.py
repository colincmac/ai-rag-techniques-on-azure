...
def _exp_growth_func(x, a, b):
    return a * np.exp(b * x)

def _logistic_growth_func(time: np.ndarray[int], K: int, P0: int, r: float):
    """
...
def forecast_exp_growth(years: np.ndarray[int], metric: np.ndarray[int], forecast_years: np.ndarray[int]):
    """
    years: array of historical years for user data
    """
    params, pcov = curve_fit(_exp_growth_func, years - years[0], metric)
    forecast = _exp_growth_func(forecast_years - years[0], *params)
    return forecast
...
def forecast_logistic_growth(years: np.ndarray[int], users: np.ndarray[int], forecast_years: np.ndarray[int], current_growth_rate: float, est_tam: int):
    ...
    params, pcov = curve_fit(_logistic_growth_func, years - years[0], users, p0=[est_tam, users[0], current_growth_rate], bounds=bounds)
    forecast = _logistic_growth_func(forecast_years - years[0], *params)
    return forecast



class ScenarioAssumptions(TypedDict):
